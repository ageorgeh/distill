...
function retriever(counter?: { calls: number }): GortexContextProvider {
  return { gather: async () => {
    ...
    return { status: "complete", text: "graph candidates", durationMs: 5, bytes: 16, rawBytes: 16, truncated: false, command: ["gortex", "explore"], retrievalTaskBytes: 20, explicitReferenceCount: 0, gitReferenceCount: 0, selectedGitReferences: [], gitSeedDecisions: [], indexStatus: { status: "fresh" }, indexCommand: ["gortex", "repos", "--json"], supplementedReferences: [], documentationIndexes: [], deterministicEvidenceBytes: 0 };
  } };
}
...
describe("single-response context gathering", () => {
  ...
  it("makes merge-review Git locations mandatory retrieval seeds and passes documentation evidence", async () => {
    ...
    try {
      ...
      const reviewRetriever: GortexContextProvider = { gather: async (request, options) => {
        ...
        return { status: "complete", text: "review candidates", durationMs: 1, bytes: 17, rawBytes: 17, truncated: false, command: ["gortex", "explore"], retrievalTaskBytes: 42, explicitReferenceCount: 1, gitReferenceCount: 1, selectedGitReferences: ["worker.ts"], gitSeedDecisions: [], indexStatus: { status: "fresh" }, indexCommand: ["gortex", "repos", "--json"], supplementedReferences: ["worker.ts"], documentationIndexes: ["docs/llms.txt"], deterministicEvidenceBytes: 12 };
      } };
      const gather = createContextHandler(resolveConfig({}), {
        provider: provider({ calls: 0 }),
        retriever: reviewRetriever,
        gitContext: { gather: async () => ({ references: ["worker.ts"], changes: [{ path: "worker.ts", additions: 1, deletions: 0, sources: ["committed"] }], text: "review patch", commands: [["git", "diff"]], truncated: false, patchReferences: ["worker.ts"] }) },
        telemetryDirectory: telemetry,
        resolveLimit: async () => ({ limit: 24_000, source: "default" }),
      });
      await gather({ action: "gather", workspaceRoot: root, intent: "merge-review", objective: "Review this branch.", references: ["Task 085"] });
  ...
  it("continues through Spark and reports a degraded graph retrieval", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "distill-context-degraded-"));
    const telemetry = await mkdtemp(path.join(tmpdir(), "distill-context-degraded-telemetry-"));
    try {
      await writeFile(path.join(root, "worker.ts"), "export const worker = true;\n", "utf8");
      const degraded: GortexContextProvider = { gather: async () => ({
        status: "degraded", failure: "Gortex explore timed out after 60000ms.", text: "deterministic candidates", durationMs: 60_000,
        bytes: 24, rawBytes: 0, truncated: false, command: ["gortex", "explore"], retrievalTaskBytes: 20,
        explicitReferenceCount: 1, gitReferenceCount: 0, selectedGitReferences: [], gitSeedDecisions: [],
        indexStatus: { status: "stale", headCommit: "head", indexedCommit: "old" }, indexCommand: ["gortex", "repos", "--json"],
        supplementedReferences: ["worker.ts"], documentationIndexes: [], deterministicEvidenceBytes: 10,
      }) };
      const counter = { calls: 0 };
      const gather = createContextHandler(resolveConfig({}), { provider: provider(counter), retriever: degraded, telemetryDirectory: telemetry, resolveLimit: async () => ({ limit: 24_000, source: "default" }) });
      const result = await gather({ action: "gather", workspaceRoot: root, intent: "advise", objective: "Assess worker.", references: ["worker.ts"] });
      expect(counter.calls).toBe(1);
      expect(result).toContain("CONTEXT NOTICE Graph retrieval degraded");
      const files = await readdir(path.join(telemetry, "invocations"));
      const stored = JSON.parse(await readFile(path.join(telemetry, "invocations", files[0]!), "utf8")) as any;
      expect(stored.gortex).toMatchObject({ status: "degraded", failure: "Gortex explore timed out after 60000ms.", indexStatus: { status: "stale" } });
    } finally { await rm(root, { recursive: true, force: true }); await rm(telemetry, { recursive: true, force: true }); }
  });
