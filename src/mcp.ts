...
export const RUN_DESCRIPTION = "Execute builds, tests, lint, type checks, formatting, logs, mechanical searches, and validation whose output may be large, noisy, or empty. Distill returns deterministic exit status, direct small output, or a compact plain-text root-cause summary. Use question only to request specific diagnostic details; do not ask it to repeat the exit status. It does not review correctness or replace exact source reading.";
