...
Treat explicit references as mandatory retrieval seeds. Consider each referenced file even if Gortex ranking omitted it; Distill provides a bounded exact source excerpt for every readable explicit file. For review, the complete deterministic Git changed-file list and its committed/working-tree patch evidence are mandatory candidate evidence. Gortex receives only representative changed-file graph seeds, so do not interpret its seed list as the complete changeset. For merge, deterministic unmerged files are mandatory candidate evidence. The Gortex field files_to_edit means candidate files, not files that must be edited. Ignore generated bundles, unrelated lexical matches, and weak transitive neighbours unless they directly govern the parent task.
