...
The capable agent reads and understands the authoritative task, review finding, specification, or merge request first. It then calls context gather with a complete repository-context objective. Distill prepares explicit-reference and documentation evidence, runs one bounded graph-only `gortex explore` in TOON format, and gives that precomputed candidate context to Spark. Supplied inline evidence participates in the Gortex query. `merge-review` requests add the complete deterministic base-to-HEAD changed-file list and a bounded patch; merge requests add unmerged files and conflict patches. Gortex receives at most six high-confidence changed-file neighbours, while Spark receives the complete deterministic Git evidence. Spark only selects verified files and ranges; it has no shell or graph tools. Distill reads the selected ranges from disk and returns their exact source in the same tool response.

Gather runs Gortex explore at most once and Spark exactly once, with no discovery follow-up, and returns one flat, deduplicated source bundle. It records Gortex index freshness before retrieval. A stale index, timeout, or ordinary Gortex error degrades graph enrichment but does not discard deterministic Git, explicit-reference, or documentation evidence; the returned context and telemetry carry a notice. Caller cancellation still aborts. Treat `EXACT SOURCE` as already read and do not repeat completed searches. Make only targeted follow-up reads when editing needs surrounding code or newly discovered details.
...
`run` executes builds, tests, lint, formatting, type checks, logs, and mechanical searches. Set its working directory with `workspaceRoot` or the shorter `cwd` alias. Use `command` for one command or a genuinely dependent shell pipeline. For multi-stage validation, use `commands: [{ name, command }]`: stages run sequentially and every stage runs even when an earlier one fails. Distill reports deterministic aggregate and per-stage status, such as `FAIL stages=4 failed=1`, so `question` should normally be omitted. Reserve it for a short extraction focus that differs from the default—not exit-status requests, command restatement, task narrative, prior work, or exclusions.
